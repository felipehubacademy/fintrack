/**
 * API: Create Solo Account
 * Cria organização + usuário admin usando função create_solo_account
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      const { data, error } = await supabaseAdmin.rpc('create_solo_account', {
        p_org_id: orgId,
      ...
      if (error) {
        console.error('❌ [API] Erro ao executar create_solo_account:', error);
        await supabaseAdmin.auth.admin.deleteUser(adminId);
        return res.status(500).json({
          success: false,
          error: 'Erro ao criar conta solo',
      ...
      console.error('❌ [API] Erro inesperado na criação da conta solo:', rpcErr);
      await supabaseAdmin.auth.admin.deleteUser(adminId);
      return res.status(500).json({
        success: false,
        error: 'Erro ao criar conta solo',
    ...
    console.error('❌ [API] Erro inesperado ao criar conta solo:', err);
    return res.status(500).json({
      success: false,
