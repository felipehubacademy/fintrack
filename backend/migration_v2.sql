-- =====================================================
-- FINTRACK V2 - MIGRAÇÃO (ADAPTANDO TABELAS EXISTENTES)
-- =====================================================
...
DECLARE
    ...
    felipe_center_id UUID;
    leticia_center_id UUID;
    shared_center_id UUID;
...
BEGIN
    ...
    -- Criar centros de custo padrão (um por vez)
    INSERT INTO cost_centers (id, organization_id, name, type, color)
    VALUES (gen_random_uuid(), default_org_id, 'Felipe', 'individual', '#3B82F6')
    RETURNING id INTO felipe_center_id;
    
    INSERT INTO cost_centers (id, organization_id, name, type, color)
    VALUES (gen_random_uuid(), default_org_id, 'Letícia', 'individual', '#EC4899')
    RETURNING id INTO leticia_center_id;
    
    INSERT INTO cost_centers (id, organization_id, name, type, color)
    VALUES (gen_random_uuid(), default_org_id, 'Compartilhado', 'shared', '#8B5CF6')
    RETURNING id INTO shared_center_id;
    
    -- Buscar categoria padrão
    SELECT id INTO default_category_id FROM budget_categories WHERE name = 'Outros' LIMIT 1;
    ...
    UPDATE expenses SET
        ...
        cost_center_id = CASE 
            WHEN owner = 'Felipe' THEN felipe_center_id
            WHEN owner = 'Leticia' OR owner = 'Letícia' THEN leticia_center_id
            WHEN owner = 'Compartilhado' THEN shared_center_id
            ELSE felipe_center_id -- Default para Felipe
        END,
    ...
    RAISE NOTICE 'Migração concluída: % despesas migradas', (SELECT COUNT(*) FROM expenses WHERE organization_id = default_org_id);
END;
$$ LANGUAGE plpgsql;
